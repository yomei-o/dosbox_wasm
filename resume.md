...
ただしその直後から**画面の更新が止まる**。描画ピクセル数が 1 ドットも
変わらなくなり、スクリーンショットは古いフレームが残って破れて見える。

**ハングではない。** `dosbox_x_cpu_probe` で見ると CPU はセグメント
`28b3` / `3603` / `0019` を絶えず行き来していて、どれもコールバック領域の
外＝実際のゲストコード。MSIME も JW_CAD も走り続けている。**止まっている
のは描画だけ。**

次に疑うのは `GFX_EndUpdate()`（`src/gui/sdlmain.cpp` の 3311 行あたり）。
ここにも `emscripten_sleep(0)` が入っていて、`CALLBACK_Idle` から yield を
外したのと同じ問題がフレーム提示側に残っている。MSIME が DOS/V の表示
ドライバ経由で描き始めた時点でその経路に入り、以後フレームが出なくなった
という筋が通る。

フレームが出ているかは `GFX_EndUpdate` の呼び出し回数をエクスポートすれば
すぐ分かる（`dosbox_x_cpu_probe` と同じ要領）。
