...
## 11. 図面を SVG / PDF で取り出す（出力の採取まで完了）2026-09-17
...
→ 「出力ファイル名 ?」 に名前 + Enter
→ 設定画面   [ESC] 設定 |1)確定(L)|2)範囲変更(R)|...     → 作図領域を左クリック
→ 作図開始   [ESC] 作図開始 |1) 実行(L)|2) 中止(R)|...   → 作図領域を左クリック
→ 出力
```

**要点は `(L)` `(R)`。** 上部バーの項目に `(L)` と書いてあれば、バーを狙う
必要はなく**作図領域を左クリックすればよい**。`1` キーでは選べない。
ここを取り違えて何度も空振りした。

### 観測性が鍵だった（同じ轍を踏まないこと）

長く詰まった原因は、エミュレータの速度ではなく**画面が読めていなかった**こと。

Playwright の `canvas.screenshot()` は **CSS で拡大されたキャンバス**を撮るので、
画像が継ぎはぎになり歪む。そこから座標を測っても当たらない。

**キャンバスのバッキングストアから直接取る**こと。ちょうど 640x480 で、
1ピクセルがゲストの1ピクセルに対応する:
...
これに切り替えて上部バーを2倍に拡大したら、`1)確定(L)` と読めて即座に解けた。

### 採れた出力

`test/plot-rectangle.txt`（矩形4本、203バイト、**改行は CRLF**）:

```
B -42050 -29700 42050 29700 a1     図面範囲と用紙名
Y 1                                 線種
W 2                                 ペン
M -34280 -4335                      移動
D -10928 -4335                      描画
...
E -42050 -29700 42050 29700         終了
```

気づき:

- `Y n`（線種）は各要素の前に、しかも重複して何度も出る。変換側は**連続する
  同じ指定を読み飛ばす**か、状態として持てばよい
- 座標は `unit_x=100`（1mm = 100単位）で、`web/wasm.jwp` で決めた値
- 用紙が `a1` で出た。図面は A-3 だったので、`P_SIZE` の並びと JW_CAD の
  用紙指定の対応は**未確認**。範囲は `B` 行と `E` 行に入っているので、
  変換はそちらを使えば用紙名に依存しない

### できたもの

`web/plot.js` が解析して、3通りに書き出す:

| | 実装 | 検証 |
|---|---|---|
| PNG | SVG をキャンバスに描いて `toBlob` | ✓ 1200/2400px |
| SVG | 直接生成、文字は `<text>` | ✓ 矩形で通し確認 |
| PDF | libharu を wasm 化（`web/pdf.wasm`、222KB） | ✓ MediaBox と内容ストリームを実測 |

PDF の日本語は **`90ms-RKSJ-H`**（PDF 標準の CJK エンコーディング）で出すので、
**フォントを埋め込まない**。プロッタ出力の文字はシフトJISのまま渡せる。
そのため解析はバイトのまま行い、復号は書き出し側でやる（SVG は文字が要る、
PDF はバイトが要る）。

用紙は**描かれた中身の範囲＋10mm 余白**に合わせる。プロッタ設定の用紙選択は
`.JWP` の宣言順で決まってしまい、A3 の図面が A1 の紙の左下に小さく乗る、
という状態になったため（利用者の指摘で判明）。

### 文字だけ通っていない（要確認）

**実装はしてあるが、一度も実際の文字で検証できていない。**

分かっていること:

- ブラウザの入力欄から JW_CAD の「文字列入力」欄に文字は**届く**
  （`ABC123` が入っているのを画面で確認済み）
- そこから**図面に配置するところが自動操作で再現できなかった**。
  Enter で確定 → 位置をクリック、を試したが図面に文字が出ない
- 結果、プロッタ出力に `A` / `N` / `K` 行が一度も現れていない

利用者は手作業で日本語を書き込めている（確認済み）ので、**機能としては
あるはず**。足りないのは自動操作の手順だけ。

**一番早い確かめ方**: 手で文字を書いた図面を1枚作り、プロッタ出力して
PNG / SVG / PDF を出す。`A` / `N` / `K` が出力に現れれば変換側は動く
（コードは書いてある）。PDF の日本語がリーダーでどう出るかもそこで分かる。

### 次にやること

1. 上の文字の確認
2. 文字の座標・サイズ・回転角の効き方を実データで確かめる
